...
export default {
  mounted () {
    ...
    const accessToken = 'pk.eyJ1IjoiYnJ1Y2VsZWUiLCJhIjoiQkZtY0dJMCJ9.a7zFI-MdJbKT9SGLSDFHeQ'

    //* * MODIFY THIS SECTION
    // Specify YOUR uploaded Mapbox Studio style URL
    const styleURL = 'mapbox://styles/brucelee/cja709h6k4zhb2ss3dhilexjt'
    const mapId = 'brucelee.cd-115-2017' // used by the click handler only
    ...
    $('#state').change(function () {
      if (this.value === '') {
        ...
        const newHash = 'state=' + this.value
    ...
    if (mapboxgl.supported({ failIfMajorPerformanceCaveat: true })) {
      ...
      map.on('load', function () {
        ...
        function focusMap (stateAbbr, districtCode) {
          ...
          for (let i = 1; i <= 5; i++) {
            ...
            let exisitingFilter = map.getFilter('districts_' + i)
            ...
            const layerFilter = filter.concat([exisitingFilter])
          ...
          const districtAbbr = districtCode || ''

          // Determine the best center and zoom level for the new map focus and then go there
          const view = geoViewport.viewport(bboxes[stateAbbr + districtAbbr], [width / 2, height / 2])
          map.jumpTo(view)
        }
        ...
        function checkHash () {
          ...
          if (window.location.hash) {
            ...
            const hashData = hash.substring(1).split('&').map(function (d) { return d.split('=') })
        ...
        map.on('click', function (e) {
          ...
          if (district) {
            ...
            alert(`That's ${district.state} - ${district.number}, i.e. ${district.NAMELSAD}.`)
          } else {
      ...
      // eslint-disable-next-line no-undef
      const map = L.mapbox.map('map').setView(continental.center, continental.zoom)
      // eslint-disable-next-line no-undef
